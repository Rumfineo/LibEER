...
class MHGCN(nn.Module):
    ...
    def __init__(self, layers, dim, chan_num, band_num, hidden_1, hidden_2):
        ...
        for _ in range(layers):
            self.HGCN_layers.append(DGCN(channels=self.band_num, num_nodes=self.chan_num,
...
class MLPEncoder(nn.Module):
    """Two-layer MLP encoder for the w/o TDGCN ablation."""

    def __init__(self, in_planes=[5, 62], layers=2, hidden_1=256,
                 hidden_2=64, class_nums=3):
        super(MLPEncoder, self).__init__()
        self.input_dim = int(in_planes[0] * in_planes[1])
        self.fc1 = nn.Linear(self.input_dim, hidden_1)
    ...
    def forward(self, x):
        ...
        if x.size(1) != self.input_dim:
            raise ValueError(
                f"MLPEncoder expected flattened input dim {self.input_dim}, got {x.size(1)}"
            )
        out = self.fc1(x)
        out = F.relu(out)
        out = self.dropout1(out)
...
class DomainAdaptationModel(nn.Module):
    ...
    def __init__(self, in_planes=[5, 62], layers=2, hidden_1=256,
        ...
        # 组件初始化
        self.encoder = self._build_encoder(in_planes, layers, hidden_1, hidden_2, num_of_class)
    ...
    def _build_encoder(self, in_planes, layers, hidden_1, hidden_2, num_of_class):
        return self.encoder_cls(
            in_planes=in_planes,
            layers=layers,
            hidden_1=hidden_1,
            hidden_2=hidden_2,
    ...
    def _get_source_similar(self, feature_source_f, source_label_feature, source_label, source_index):
        """
        计算源域相似性
        ...
        bank_index = source_index.detach().cpu().long()
        score_index = source_index.to(self.device).long()

        # 更新特征和得分库
        self.source_f_bank[bank_index] = output_f.detach().clone().cpu()
        self.source_score_bank[score_index] = source_label_feature.detach().clone()
        self.source_label_bank[bank_index] = source_label.detach().clone().view(-1).cpu()

        # 计算类别原型
        prototype_class = []
        for class_id in range(self.num_of_class):
            valid_labels = self.source_label_bank >= 0
            source_feature = self.source_f_bank[valid_labels & (self.source_label_bank == class_id)]
    ...
    def _get_target_similar(self, feature_target_f, target_label_feature,
        ...
        bank_index = target_index.detach().cpu().long()
        score_index = target_index.to(self.device).long()

        # 更新目标域特征库
        self.target_f_bank[bank_index] = f.detach().clone().cpu()
        self.target_score_bank[score_index] = target_label_feature.detach().clone()
    ...
    def get_init_banks(self, source, source_index, source_label):
        """
        初始化源域特征库
        ...
        with torch.no_grad():
            ...
            self.source_f_bank[bank_index] = F.normalize(source_f).detach().clone().cpu()
            self.source_score_bank[score_index] = source_label_feature.detach().clone()
            self.source_label_bank[bank_index] = source_label.detach().clone().view(-1).cpu()
    ...
    def get_init_banks_tgt(self, tgt, tgt_index):
        ...
        with torch.no_grad():
            ...
            bank_index = tgt_index.detach().cpu().long()
            score_index = tgt_index.to(self.device).long()

            self.target_f_bank[bank_index] = F.normalize(tgt_f).detach().clone().cpu()
            self.target_score_bank[score_index] = tgt_label_feature.detach().clone()
...
class DomainAdaptationMLPModel(DomainAdaptationModel):
    """PCL domain-adaptation model with a two-layer MLP encoder."""
